...
| **`chats/<角色>/<對話>.jsonl`＋標頭行** | 我們已經在做，而且已經寫得出有效的酒館聊天檔。 |
...
**現況**：`art/characters/<卡 id>/`、`art/worldbooks/<書 id>/`、`art/chats/<角色>/<對話名>/`、`art/tavern/`；資料夾內容即清單；主圖記在 `tavern.json`。
...
| 順位 | 事情 | 為什麼是這個順位 |
|---|---|---|
| **P1** | **原子寫入**（temp file + rename，用在 `tavern.json`、卡片、世界書） | **新發現，而且可能是最便宜的一條**。ST 幾乎所有寫入都走 `write-file-atomic`。我們現在是裸的 `writeFile`——斷電／當機剛好落在寫入中間，**使用者的卡就變成半個檔案**。這是一行級的修改，卻是我們目前最實在的資料安全漏洞。（注意：我們的 `createChat` 已經用 `flag: 'wx'`，那是對的，保留。） |
...
`createChat` 用 `flag: 'wx'`（獨佔建立，撞名就試下一個編號）、
