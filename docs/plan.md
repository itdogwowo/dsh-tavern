...
| 世界書 | ✅ 位置三個（2.6.59）、房間那一層（2.6.62）、逐書覆寫（2.6.64）、**房間調條目優先序（2.6.71）**、**「✎ 修改」跳到藏書頁（2.6.72）** |

### 2.6.72 這一輪（房間那一頁的兩個入口）

使用者：

> ▸ 條目，這個按鈕其實可以是跳轉去修改　然後摺疊功能改為整條都可以摺疊

- **整條列**＝展開／收起（看條目、調**這一間房**的優先序）；**「✎ 修改」**＝跳到酒館的
  「📖 藏書」打開那一本（在那裡改標題、關鍵字、`order` 與**內容**）。
- ⚠️ **「✎ 修改」是 2.6.66 做過、2.6.68 拿掉的那一條路**——實作沿用
  `pendingBookOpen` ＋ `bookOpenChannel` ＋ `jumpToBook()`（三件事缺一不可，
  少一件的症状是**什麼都沒有發生**）。⚠️ `pendingBookOpen` **一定要宣告在檔案最前面**
  （`var` 提升只提升宣告；這一型踩過三次，其中一次是整頁白屏）。
- ⚠️ 列上的控制項用 `.dsh-tv-rowCtl`（`display:contents`）**攔掉冒泡**，不然按開關
  會順手把整列展開。**鍵盤**也要能用（`role="button"` ＋ Enter／Space）。
- ⚠️ 折疊箭頭用 `transform` 旋轉，**不是換字**（換字會讓那一格寬度改變 ⇒ 整排左右跳）。
