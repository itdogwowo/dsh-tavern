...
| | |
|---|---|
| 版本 | **2.6.0**（四分區；**還沒 commit**——Windows 這台沒有 git）。GitHub `main` 上還是 2.5.0 |
...
| 安裝 | ✅ `link:` 指回工作區（改工作區＝改插件；不再有兩份實體複製）。**2026-09-18 之後兩台都是**，Windows 那台這天補上的（§6.1c） |
| UI | **四分區完成（2.6.0）**：🏠 大廳／💬 包廂／🎭 卡司／📖 藏書，在同一張面板裡切換、一次只畫一區；卡司是海報牆→編輯器，藏書有條目清單。對話頁能聊天、刪除對話有入口 |
| 酒館 | 一間（使用者自己選的資料夾；附預設的老闆娘＋世界書，**還沒有任何對話**） |

**一句話**：**引擎做好了、油門接上了、車也真的會跑了——四分區的車殼也裝好了（§13，2.6.0）。**
...
見 §7.5。四個分區在同一張 `main:tavern` 面板裡切換、一次只畫一區；
`currentZone` 走模組層級變數（理由與測試出口見 §7.5）。

真瀏覽器（ego-browser）驗過：

- 四個分區各切一次，**每次都只亮一個分頁**、內容正確換過來。
