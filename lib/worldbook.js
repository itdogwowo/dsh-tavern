 ...
 * | `order`（條目）排序 | ✅ | |
 * | **這一間房對個別條目 `order` 的覆寫**（2.6.71） | ✅ **我們的** | ST 沒有「這一場的順序」；見下面那一節 |
...
// ⚠️ **這裡記著走錯的一步，不要重犯。**
//
// 2.6.71 的第一版加了**書層**的 `priority`（一本書一個數字、整本排在別本前面），
// 理由是「房間不該改書的內容，所以缺一個『這一本比那一本重要』的槓桿」。
// 那是一個**我自己發明的抽象**，而使用者要的是**他已經看得到的東西**：
//
//   > 等等為什麼多了這些東西，我說的次序可能和你說的次序不一樣，
//   > 我看見**世界書裏面有不同的項目設定次序**，我說的是那個，
//   > 你獨立給了我另一個次序了
//
// 他看得到的是**條目自己的 `order`**（「📖 藏書 → 點一本書 → 每一條條目」下面
// 標著「優先序」的那個數字，ST 的欄位）。所以這一輪做的是那個：
// **房間可以調每一條條目的優先序，而一個字都不改書的檔案。**
//
// > **教訓**：要加一個新的抽象之前，先確認使用者指的是**畫面上哪一格**。
// > 他講「優先序」的時候，畫面上已經有一個叫「優先序」的欄位了。
//
// ## 形狀（存在 `room.json` 的 `worldbookEntryOverrides`）
//
//     { "酒館": { "0": { "order": 950 } } }
//
//   - 第一層的鍵＝**書的 id**（`worldbooks/<id>.json`）。
//   - 第二層的鍵＝**條目的鍵**（見 `entryKeyOf`）。
//   - 值目前只有一個欄位 `order`（與 ST 同名、同方向：**大的先注入**）。
//
// ⚠️ **沒有列在上面的條目 ＝ 完全照書自己的 `order`**（所以既有房間一字不差）。
//
// ⚠️ **鍵用 `uid`，不是陣列索引**：`uid` 是 ST 給條目的識別碼，改內容、改關鍵字
// 都不會動到它；用索引當鍵的話，書裡**插一條**就會讓整份覆寫**位移到別的條目上**
// ——那是安靜的錯（使用者只會覺得「設定的順序自己跑了」）。沒有 `uid` 的條目
// 退回 `#<索引>`，而那種書本來就沒有可靠的識別碼可用。
//
// ⚠️ **代價**：`order` 決定誰**先吃掉預算**（先到先得，見 `collectLore`）。
// 把一條調到最前面，就是讓它在預算不夠時優先留下來。

/** 條目 `order` 的合法範圍（**與 ST 同一個欄位**，不是我們新發明的）。 */
export const ORDER_LIMITS = { min: -9999, max: 9999 }

/**
 * 一個 `order` 值 → 整數，或 `null`（＝沒有指定／不合法）。
 *
 * ⚠️ **回 `null` 而不是 0**：`0` 是一個**合法的 `order`**（ST 的慣例裡 0～19 是
 * 「錦上添花」那一級）。把「不合法」與「指定 0」混成同一個答案，就再也分不出
 * 兩者——而那正是「還原成書自己的值」的判斷依據。
 *
 * ⚠️ **收數字字串**（`'950'`）：`<input type="number">` 的 `value` 是字串，
 * 而驗證只該有一份（宿主半）。客戶端不必自己 `parseInt` 一次。
 */
export function normalizeOrder(value) {
  ...
  if (parsed < ORDER_LIMITS.min || parsed > ORDER_LIMITS.max) return null
  return parsed
}

/**
 * 一條條目的**穩定鍵**（`worldbookEntryOverrides` 的第二層鍵）。
 *
 * ⚠️ 這一條規則**兩邊都要一樣**：宿主半用它存與讀，而房間那一頁要靠它送 patch。
 * 客戶端拿的是 `worldbook.roomEntries` 回的 `key`（**宿主半算好給它**），
 * 所以它不必自己鏡射這條規則——這也是為什麼那一支 op 存在。
 *
 * @param entry - 一條條目。
 * @param index - 它在**正規化之後**清單裡的位置（沒有 `uid` 時才用到）。
 * @returns 字串鍵。
 */
export function entryKeyOf(entry, index) {
  const uid = entry !== null && typeof entry === 'object' ? entry.uid : undefined
  if (typeof uid === 'number' && Number.isFinite(uid) && Number.isInteger(uid)) return String(uid)
  return `#${String(index)}`
}

/** 這一間房對**這一本書的條目**的覆寫（`{}` ＝ 一條都沒調）。 */
export function entryOverridesOf(entryOverrides, bookId) {
  const mine = entryOverrides !== null && typeof entryOverrides === 'object' && Array.isArray(entryOverrides) === false
    ? entryOverrides[bookId]
    : undefined
  return mine !== null && typeof mine === 'object' && Array.isArray(mine) === false ? mine : {}
}

/**
 * 條目的 `order`：**這一間房的覆寫 → 書自己的 → ST 的預設 100**。
 *
 * ⚠️ `100` 是 ST 自己對「沒有 `order` 的條目」用的預設值（`collectLore` 以前就
 * 是寫死這個數字），不是我們挑的。
 */
export function orderOf(entry, index, entryOverrides, bookId) {
  const mine = entryOverridesOf(entryOverrides, bookId)[entryKeyOf(entry, index)]
  const fromRoom = mine !== null && typeof mine === 'object' && Array.isArray(mine) === false ? normalizeOrder(mine.order) : null
...
//
// ⚠️ **條目層的順序住在另一格**（`worldbookEntryOverrides`，見上一節）：
// 「這一場用不用這本書」與「這一場裡某一條排多前面」是兩件事，
// 混在同一個物件裡會讓「整份覆寫取代」的 patch 變得很難寫對。
//
...
// ⚠️ **優先序（2.6.69 改的）**：
//
//     房間對這一本書的指定  →  書自己指定的  →  這一間房的預設  →  酒館的預設  →  `in-chat`
//
// 「這一間房」比「這本書的預設」更窄、也更晚決定，所以它贏——那正是「房間可以
// 設定位置」這句話的意思（使用者：「我在房間可以設定位置以及層級，預設是跟酒館」）。
// 2.6.64–2.6.68 的順序相反（書贏），結果是**書自己指定過的那一本，房間怎麼選都
// 不會生效**——那一格能改卻改不動，只好鎖起來。改成房間優先之後就沒有這種格子了。
//
// ⚠️ 只有「這一間房真的設了那一本書」時才不一樣：沒設定的房間行為一字不差。
...
export function applyBookOverride(book, overrides) {
  ...
  //    ——`positionOf()` 讀的就是它，而它排在**最前面**（這一間房的指定最優先）。
  const data = book?.data !== null && typeof book?.data === 'object' && Array.isArray(book.data) === false ? book.data : {}
  return { ...book, data: { ...data, positionOverride: position } }
}

/**
 * 條目的顯示名（給訊息與 UI 用；`comment` 是 ST 的欄位）。
 *
 * ⚠️ **這一條規則只有一份**：房間那一頁的「▸ 內容」清單也用它（宿主半算好
 * 隨 `worldbook.roomEntries` 一起回去）。客戶端以前自己算過一份
 * （`comment` → `name`），兩份的差別是**安靜的**：兩邊畫出來的標題不一樣，
 * 而使用者會以為自己看錯。V2 方言的標題欄位叫 `name`，所以兩個都要吃。
 */
export function entryLabelOf(entry) {
 ...
 * @param options - `{ caseSensitive, matchWholeWords, budgetChars, roomPosition,
 *   defaultPosition, bookOverrides, entryOverrides }`。
 ...
 *
 * ⚠️ **2.6.71 起 `order` 可能是「這一間房的覆寫」**（`options.entryOverrides`）。
 * 沒給覆寫時**完全等於舊行為**（`orderOf` 回書自己的值）。
 */
export function collectLore(books, scanText, options) {
  ...
  const bookId = (raw) => (typeof raw?.id === 'string' ? raw.id : '')
  ...
  for (const raw of Array.isArray(books) ? books : []) {
    ...
    const list = normalizeEntries(book?.data)
    for (let index = 0; index < list.length; index += 1) {
      ...
      picked.push({
        ...
        /** 這一條的穩定鍵——房間的覆寫就是用它對起來的（見 `entryKeyOf`）。 */
        key: entryKeyOf(entry, index),
        /**
         * ⚠️ **`order` 已經把房間的覆寫算進去了**（2.6.71）：
         * 這一間房的指定 → 書自己的 → ST 的預設 100。見 `orderOf`。
         */
        order: orderOf(entry, index, entryOverrides, id),
        /** 書自己的值（沒有房間覆寫時就是它）——房間那一頁要靠它做「還原」。 */
        ownOrder: typeof entry.order === 'number' ? entry.order : 100,
        label: entryLabelOf(entry),
        content,
        reason,
 ...
 *
 *   1. **這一間房對這一本書的指定**（`room.json` 的 `worldbookOverrides[id].position`）
 *   2. **這本書自己的 `position`**（存在書的檔案裡，「📖 藏書」改的就是它）
 *   3. `room.json` 的 `worldbookPosition`（**這一間房的預設**）
 *   4. `tavern.json` 的 `worldbookPosition`（**這一間酒館的預設**）
 *   5. `'in-chat'`——⚠️ **必須是這個**，因為它與 2.6.58 以前的行為一字不差
 *
 * ⚠️ **1 與 2 的順序是 2.6.69 定的**（在那之前是「書贏」）：房間是更窄也更晚
 * 決定的那一層，所以「我在房間可以設定位置」對**每一本**都要成立。見上面
 * `applyBookOverride` 那一段的說明。
 *
...
export function positionOf(data, roomFallback, tavernFallback) {
  ...
  /**
   * ⚠️ **`positionOverride` 排在最前面**：這一間房對這本書的指定（見
   * `applyBookOverride`）。它是**影子欄位**——刻意不叫 `position`，因為它不可以
   * 蓋掉書檔案裡的那個值（房間**不改使用者的檔案**，只是這一場讀的時候換一個）。
   *
   * 它贏過書自己的 `position` 是刻意的：房間是更窄、更晚決定的那一層
   * （使用者：「我在房間可以設定位置以及層級，預設是跟酒館」）。
   * 只有在這一間房**真的指定過這一本書**時才會有它。
   */
  const fromRoomBook = normalizePosition(book.positionOverride)
