 ...
 * 那半的版本，再跟磁碟上的值比對。
 */
export const TAVERN_BUILD = 'tavern-2.6.2'
...
export function apply(ctx) {
  ...
  const routes = {
    ...
    /* 對話紀錄 */
    'chat.list': async (args) => (await registry.requireById(args?.id)).workspace.listChats(),
    'chat.create': async (args) =>
      (await registry.requireById(args?.id)).workspace.createChat(args?.character, args?.name),
    'chat.delete': async (args) =>
      (await registry.requireById(args?.id)).workspace.deleteChat(args?.character, args?.chat),
    /**
     * 改一份對話的名字。
     *
    ...
    'chat.rename': async (args) =>
      (await registry.requireById(args?.id)).workspace.renameChat(
        args?.character,
        args?.chat,
        args?.name,
      ),
    'chat.append': async (args) =>
      (await registry.requireById(args?.id)).workspace.appendChatMessages(
        args?.character,
        args?.chat,
        args?.messages,
      ),
    'chat.messages': async (args) =>
      (await registry.requireById(args?.id)).workspace.readChatMessages(args?.character, args?.chat),
  ...
  ctx.effect(() => {
    const dispose = ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          // 這幾個 op 的 body 是檔案本身（圖片、PNG 卡、世界書檔），不能先當 JSON 解析。
          const binary = op === 'assets.write' || op === 'character.import' || op === 'worldbook.import'
