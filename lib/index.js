 ...
 * 那半的版本，再跟磁碟上的值比對。
 */
export const TAVERN_BUILD = 'tavern-2.6.71'
...
export function apply(ctx) {
  ...
  const routes = {
    ...
    workspace: async () => {
      ...
      if (found === undefined) {
        const listed = await registry.list()
        return { root: '', name: '', exists: false, counts: {}, settings: null, render: null, formatBooks: [], layout: SUBDIRS, ...listed }
      }
      const summary = await found.workspace.summary()
      const settings = await found.workspace.readSettings()
      /**
       * ⚠️ 回覆格式（`render.json`）**也掛在這裡**，理由與 `settings` 一模一樣：
       * 客戶端讀一次概要就要拿到「這一間酒館現在怎麼運作」的全部。
       * 少一個欄位的症狀是**安靜的**——對話頁會一直用內建的 `plain` 設定，
       * 而畫面看起來完全正常（只是模型給的格式不被讀懂）。
       */
      const render = await found.workspace.readRender()
      /**
       * ⚠️ **哪幾本世界書「也在教回覆格式」**——這是「一個東西兩個來源」的警告來源。
       *
       * 格式指令可以有兩個老師：世界書（`constant` 條目，住在**訊息**裡）與
       * `render.json`（plugin 送進**系統提示**）。兩個同時開著就會給模型兩份
       * 規格（例如 `data` 一邊說用 `text`、一邊說用 `rows`），而設定頁要看得見
       * 那件事——不能靠使用者記得。
       *
       * 讀不到就回空陣列：那只是提示，不該讓整份概要讀不出來。
       */
      const formatBooks = await found.workspace.formatSpecBooks().catch(() => [])
      const listed = await registry.list()
      return { ...summary, settings, render, formatBooks, layout: SUBDIRS, ...listed }
    },
    ...
    /**
     * 世界書的**注入位置**（2.6.65）——**讀**的那一半。
     *
     * 每一本書最後會被放在哪裡（酒館層的預設已經算進去，所以使用者看到的
     * 是「它會被放在哪」而不是「它寫了什麼」）。
     *
     * ⚠️ **寫**的那一半在 `worldbook.position`——只改那本書的 `position`
     * 一個欄位，不是整本送過來（理由寫在 `workspace.writeWorldbookPosition()`）。
     */
    'worldbook.positions': async (args) => (await registry.requireById(args?.id)).workspace.worldbookPositions(),
    /**
     * **這一間房**會把每一本世界書放在哪（書 → 房 → 酒館，三層一起算）。
     *
     * ⚠️ 與上面那一支分開是必要的：房間那一頁要顯示的是「**在這一間房裡**」
     * 的位置，而酒館那一支只算到酒館層。拿錯的那一支會顯示錯的位置
     * ——而那是**安靜的**：使用者看到一個位置、實際送去的是另一個。
     */
    'worldbook.roomPositions': async (args) =>
      (await registry.requireById(args?.id)).workspace.roomWorldbookPositions(args?.character, args?.room),
    /**
     * **這一間房裡**某一本書的每一條條目（含這一間房的優先序）。
     *
     * ⚠️ 條目的**鍵由宿主半算**（`entryKeyOf`）並隨回應回去——客戶端要送
     * 「某一條的覆寫」就得知道鍵，兩邊各算一份就是同一個東西兩個來源
     * （走散時是**安靜的**：覆寫存到不存在的鍵上、永遠不生效）。
     * 順便回內容給「▸ 內容」畫——**唯讀**，房間不改書。
     */
    'worldbook.roomEntries': async (args) =>
      (await registry.requireById(args?.id)).workspace.roomEntries(args?.character, args?.room, args?.book),
    /**
     * 改一本世界書的**注入位置**（2.6.59）。
     *
     * ⚠️ `where` 給 `null`／`''` ＝ **把書裡的 `position` 刪掉**（＝回到
     * 「跟著酒館預設」）。2.6.71 才有這個三態——在那之前畫面上寫著
     * 「請自己到原始 JSON 把 position 刪掉」，那是一個做不到的設定。
     */
    'worldbook.position': async (args) => ({
      position: await (await registry.requireById(args?.id)).workspace.writeWorldbookPosition(args?.book, args?.where),
    }),
    ...
    /* 這間酒館的回覆格式（render.json）：我們怎麼告訴模型「請這樣回」 */
    /**
     * 讀回覆格式。
     *
     * ⚠️ 這一組**不像 `theme.read` 要附預設表**：`render.json` 的預設值
     * （模式清單、可用的 kind、內建標記）在 `lib/render.js` 裡，而客戶端需要的是
     * **它自己那一份投影**（`parseConfigFromRender` 的輸出形狀）——那是純資料，
     * 由客戶端自己從這一份算出來就好。多送一份預設表＝多一個會走散的來源。
     */
    'render.read': async (args) => (await registry.requireById(args?.id)).workspace.readRender(),
    'render.write': async (args) =>
      (await registry.requireById(args?.id)).workspace.writeRender(args?.patch ?? {}),
    ...
    /**
     * 補上／更新**出貨的預設內容**（2.6.65）。
     *
     * ⚠️ 給**既有的**酒館用：新建的跑 `seed()`，而既有的永遠不會再跑一次——
     * 所以出貨內容的修正（例如新的格式說明）到不了它們手上。使用者按一下才會跑，
     * 而且**回報動了什麼**：讀取路徑寫檔是這個 repo 的硬規則所禁止的
     * （§2c：讀取順手補結構曾經讓「舊版殘留」的標記自己消失），所以不做成自動的。
     */
    'tavern.repair': async (args) => (await registry.requireById(args?.id)).workspace.repairDefaults(),
