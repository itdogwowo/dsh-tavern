 ...
 * 那半的版本，再跟磁碟上的值比對。
 */
export const TAVERN_BUILD = 'tavern-2.6.66'
...
export function apply(ctx) {
  ...
  const routes = {
     ...
     * 世界書的**注入位置**（2.6.65）。
     *
     * 讀：每一本書最後會被放在哪裡（酒館層的預設已經算進去，所以使用者看到的
     * 是「它會被放在哪」而不是「它寫了什麼」）。
     * 寫：**只改那本書的 `position` 一個欄位**，不是整本送過來——理由寫在
     * `workspace.writeWorldbookPosition()` 的註解裡（整本送要相信那一趟來回
     * 沒有掉東西，而那是使用者的 ST 檔）。
     */
