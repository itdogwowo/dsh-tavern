...
export function assetDir(kind, owner) {
  ...
  /**
   * ⚠️ **房間的圖跟著房間走**：`chats/<角色>/<roomId>/art/`——**不在 `art/` 底下**。
   *
   * 這是整個 `art/` 佈局唯一的例外，而它只是一個特例：`resolveAssetPath`（讀圖路由）、
   * `listAssets`、`writeAsset`、`deleteAsset` 全部都在最後呼叫這一支，所以**改這裡就夠**
   * ——URL 的形狀完全不變（`/api/dsh-tavern/assets/chats/<角色>/<房間id>/<檔名>`）。
   *
   * 為什麼是兩層才算：`owner` 是 `角色/房間id`。只有一層時（舊的 `角色` 或測試直接呼叫）
   * 走原本那條路，才不會把既有的 `art/chats/<角色>/…` 一起指到不存在的地方。
   */
  if (kind === 'chat' && parts.length === 2) return `chats/${parts[0]}/${parts[1]}/art`
...
/**
 * 資產 URL 的前綴，**用來串 `<img src>`**，所以結尾帶斜線。
 *
 * ⚠️ 這不是宿主半註冊路由用的字串：`index.js` 的 `TAVERN_ASSET_PATH` 刻意**不帶**
 * 斜線，因為 `webServer` 的 prefix 比對會自己補一個 `/` 再 `startsWith`。
 * 兩者看起來只差一個字元，合併就會讓所有插圖靜默 404。
 */
export const ASSET_ROUTE_PREFIX = '/api/dsh-tavern/assets/'

/**
 * **PNG 卡本體**的讀取路由前綴（`characters/<角色 id>.png`）。
 *
 * 為什麼要另一條路：PNG 卡是「卡」不是插圖——它不在 `art/` 底下，可是它的圖
 * **同時是這張卡的立繪**（沒有另外上傳插圖時，頭像、海報牆都用它）。
 * 同一條規矩：這裡帶斜線、`index.js` 的註冊字串不帶。
 */
export const CARD_ROUTE_PREFIX = '/api/dsh-tavern/card/'

/** PNG 卡在資產清單裡的名字（＝檔名，這樣「主圖」設定才指得到它）。 */
export function cardAssetName(id) {
  return `${safeAssetName(String(id))}.png`
}

/** PNG 卡本體的瀏覽器 URL。 */
export function cardUrl(id) {
  return CARD_ROUTE_PREFIX + encodeURIComponent(String(id))
}
