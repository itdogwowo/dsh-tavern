...
export const AGENT_BUILD = 'tavern-agent-2.6.42'
/** Cordis 外掛名。 */
...
/**
 * 讀酒館 `tavern.json` 裡的一個字串欄位。
 *
 * 卡片路徑固定是 `<酒館>/characters/<id>.json`，所以 `tavern.json` 就在上一層。
 * 讀不到、欄位不存在、或檔案壞掉都回空字串——**不丟錯**：這些設定不該有辦法讓
 * 一整輪對話失敗。
 */
...
function roomField(binding, key) {
  ...
  try {
    const raw = JSON.parse(
      readFileSync(join(binding.root, 'chats', binding.character, room, 'room.json'), 'utf8'),
    )
    const value = raw?.[key]
    return typeof value === 'string' ? value.trim() : ''
  } catch {
  ...
  ctx.effect(
    () =>
      ctx.systemPrompt.variable(CARD_VARIABLE, (assembly) => {
        ...
        const card = readCard(cardFile, user === '' ? tavernUserName(cardFile) : user)
        ...
        const parts = []
        const tavernPart = tavernExtras(cardFile)
        if (tavernPart !== '') parts.push(tavernPart)
        const roomPrompt = roomField(binding, 'roomPrompt')
        if (roomPrompt !== '') parts.push('# 這一場\n' + roomPrompt)
        return parts.length === 0 ? card : card + '\n\n' + parts.join('\n\n')
      }),
