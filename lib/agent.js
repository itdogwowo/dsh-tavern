...
export const AGENT_BUILD = 'tavern-agent-2.6.2'
/** Cordis 外掛名。 */
...
export function renderCard(card, userName) {
  const charName = field(card, 'name') || '角色'
...
export function apply(ctx, config) {
  ...
  const user = typeof settings.user === 'string' ? settings.user : DEFAULT_USER
  const loreOptions = {
   ...
   */
  ctx.effect(
    () => ctx.systemPrompt.variable(CARD_VARIABLE, (assembly) => readCard(resolveCardFile(assembly?.agent, file), user)),
