...
import { DEFAULT_CHARACTER_ID, DEFAULT_WORLDBOOK_ID, defaultCharacter, defaultWorldbook } from './defaults.js'
import { THEME_FILE, normalizeTheme } from './theme.js'
import {
  ASSETS_FIELD,
  assetIdFor,
  normalizeAssetId,
...
export class TavernWorkspace {
  ...
  async summary() {
    ...
    if (exists) {
      ...
      counts.chats = (await this.listChats()).length
      counts.art = await countArt(this.dir('art'))
  ...
  async readSettings() {
    const fallback = { version: 1, name: '', note: '', createdAt: '' }
    try {
  ...
  async writeTheme(patch) {
    ...
    const payload = { version: 1, base: theme.base, tokens: theme.tokens }
    await this.ensure()
  ...
  async listChats() {
    ...
    for (const dirent of perCharacter) {
      ...
      const files = (await readdir(join(base, character)).catch(() => []))
        .filter((file) => file.endsWith('.jsonl'))
        .sort()
      const resolveAssetId = makeAssetIdResolver(character)
      for (const file of files) {
        const name = file.slice(0, -6)
        const assetId = resolveAssetId(name)
        const info = await stat(join(base, character, file)).catch(() => undefined)
        chats.push({
          character,
          name,
          file,
          assetId,
          // 側邊欄那一列要畫對話室自己的小縮圖，所以插圖狀態跟檔案資訊一起回。
          assets: await this.describeEntityAssets('chat', `${character}/${name}`, settings),
          mtimeMs: info === undefined ? null : info.mtimeMs,
          size: info === undefined ? null : info.size,
        })
      }
    }
    return chats.sort((left, right) => (right.mtimeMs ?? 0) - (left.mtimeMs ?? 0))
  }

  /**
   * 開一份新對話。
   *
   * 寫出 SillyTavern 形狀的聊天檔：第一行是標頭，之後一行一則訊息。
   * 目前只有標頭（DSH 不做對話循環），但檔案本身是有效的酒館聊天檔。
   *
   * 撞名一律**不覆蓋**（`初次見面-2.jsonl`）。以前是「先 stat 看存不存在、
   * 再 `flag: 'wx'`」，那有兩個毛病：檢查與寫入之間有空窗，而且最後一次嘗試
   * 撞名時會把裸的 `EEXIST` 丟給使用者。現在交給 `createUnique` 重試。
   */
  async createChat(character, name) {
    const who = requireId(character, '角色 id')
    await this.ensure()
    const dir = this.resolveWithin('chats', who)
    await mkdir(dir, { recursive: true })
    const wanted = segmentFromName(name) || `chat-${Date.now().toString(36)}`
    const header = `${JSON.stringify({ chat_metadata: {}, user_name: 'unused', character_name: 'unused' })}\n`
    const result = await createUnique(
      (index) => {
        const title = index === 0 ? wanted : `${wanted}-${index + 1}`
        return join(dir, `${title}.jsonl`)
      },
      header,
    )
    const file = result.path.slice(dir.length + 1)
    const title = file.slice(0, -'.jsonl'.length)
    return { character: who, name: title, file, assetId: assetIdFor('chat', `${who}/${title}`) }
  }

  /**
   * 刪一份對話。
   *
   * **只刪那個 `.jsonl`。** 跟 `deleteCharacter`／`deleteWorldbook` 同一個規矩：
   * 刪除就是刪除那個東西本身，不做連帶清理。所以：
   *   - 這份對話的插圖（`art/chats/<角色>/<對話名>/`）**留著**——
   *     如果之後再開一份同名的對話，圖就回來了
   *   - DSH 那邊的 session **也留著**（我們從宿主半刪不掉它；
   *     客戶端那邊可以用 `workspace.archiveSession` 把它從清單收起來）
   *
   * **但對照表要一起清掉**：它是「關於這份對話」的中繼資料，對話沒了它就是
   * 指向空氣的指標。留著的話，之後開一份同名對話會繼承一個舊 session 的綁定。
   *
   * @returns `{ character, chat, unbound }`；`unbound` 是順手解掉的 session id。
   */
  async deleteChat(character, chat) {
    const who = requireId(character, '角色 id')
    const title = segmentFromName(chat)
    if (title === '') throw new Error('對話名稱不可為空')
    const path = this.resolveWithin('chats', who, `${title}.jsonl`)

    // 先確認真的存在——不然「刪掉了」跟「本來就沒有」在畫面上長得一樣，
    // 而使用者會以為自己刪錯了東西。
    if ((await stat(path).catch(() => undefined)) === undefined) {
      throw new Error(`找不到這份對話：chats/${who}/${title}.jsonl`)
    }
    await rm(path, { force: true })

    const unbound = []
    for (const record of await this.listSessionBindings()) {
      if (record.character === who && record.chat === title) {
        await this.unbindSession(record.sessionId)
        unbound.push(record.sessionId)
      }
    }
    return { character: who, chat: title, unbound }
  }

  /**
   * 改一份對話的名字（＝改那個 `.jsonl` 的檔名）。
   *
   * 一份對話的**身分**是「哪個角色的哪一份對話」——檔名、`.sessions` 對照表、
   * 插圖資料夾、`tavern.json` 的主圖 key，四處都記著同一個名字。所以改名不是
   * 只改檔名，四處必須一起搬，否則會留下半套狀態（插圖變成孤兒、綁定指向
   * 一個不存在的對話）。這跟刪除的規矩相反：刪除只刪本體、不做連帶清理，
   * 改名則一定要連帶搬，因為那些資料本來就**屬於這一份對話**。
   *
   * **撞名不覆蓋**：新名字已經有檔案時，`createUnique`（獨佔建立）換一個編號，
   * 回傳的 `name` 才是最後真正用的名字——呼叫端一定要用它，不要用自己送進來的。
   *
   * @returns `{ character, name, file, assetId, previous, sessionIds }`；
   *          `sessionIds` 是跟著改掉的綁定（呼叫端拿它們去同步 DSH 那邊的標題）。
   */
  async renameChat(character, chat, next) {
    const who = requireId(character, '角色 id')
    const from = segmentFromName(chat)
    const to = segmentFromName(next)
    if (from === '') throw new Error('對話名稱不可為空')
    if (to === '') throw new Error('新的對話名稱不可為空')

    const dir = this.resolveWithin('chats', who)
    const source = join(dir, `${from}.jsonl`)
    if ((await stat(source).catch(() => undefined)) === undefined) {
      throw new Error(`找不到這份對話：chats/${who}/${from}.jsonl`)
    }
    // 名字沒變就當成功（呼叫端常常是「按了 Enter 但沒改」）。
    if (to === from) {
      return {
        character: who,
        name: from,
        file: `${from}.jsonl`,
        assetId: assetIdFor('chat', `${who}/${from}`),
        previous: from,
        sessionIds: [],
      }
    }

    // 1. 內容原樣搬到新檔名。**用獨佔建立**：新名字已經有人用就換編號，不覆蓋。
    const content = await readFile(source)
    const created = await createUnique((index) => {
      const title = index === 0 ? to : `${to}-${index + 1}`
      return join(dir, `${title}.jsonl`)
    }, content)
    const name = created.path.slice(dir.length + 1).replace(/\.jsonl$/, '')

    // 2. 刪掉舊檔。用 `force` 是因為上面已經確認它存在——這裡的失敗不該讓
    //    「新檔案已經寫好了」變成一個回滾不了的錯誤。
    await rm(source, { force: true })

    // 3. `.sessions` 對照表跟著搬（不然那份對話會失去它的 session）。
    const sessionIds = []
    for (const record of await this.listSessionBindings()) {
      if (record.character === who && record.chat === from) {
        await this.bindSession(record.sessionId, { character: who, chat: name })
        sessionIds.push(record.sessionId)
      }
    }

    // 4. 插圖：資料夾名是正規化過的 `assetId`，主圖 key 用的是原始對話名。
    const settings = await this.readSettings()
    const patch = {}
    const renameAssetDir = async (kind, before, after) => {
      const base = this.resolveWithin('art', kind, who)
      const oldDir = join(base, before)
      const newDir = join(base, after)
      if (before === after) return
      if ((await stat(oldDir).catch(() => undefined)) === undefined) return
      // 目標已經有東西時不合併（會蓋掉使用者的圖）——寧可留下一個舊資料夾。
      if ((await stat(newDir).catch(() => undefined)) !== undefined) return
      await rename(oldDir, newDir)
    }
    await renameAssetDir('chats', normalizeAssetId(from), normalizeAssetId(name))
    const oldKey = `chat:${who}/${from}`
    const newKey = `chat:${who}/${name}`
    if (settings?.assets !== undefined && settings.assets !== null && settings.assets[oldKey] !== undefined) {
      const nextAssets = { ...settings.assets }
      nextAssets[newKey] = nextAssets[oldKey]
      delete nextAssets[oldKey]
      patch.assets = nextAssets
    }
    if (Object.keys(patch).length > 0) await this.writeSettings(patch)

    return {
      character: who,
      name,
      file: `${name}.jsonl`,
      assetId: assetIdFor('chat', `${who}/${name}`),
      previous: from,
      sessionIds,
    }
  }
   ...
   * 把一輪對話追加到 `.jsonl`。
   *
   * SillyTavern 的訊息行形狀（只寫我們真的知道的欄位）：
   * ```json
   * {"name":"阿明","is_user":true,"is_system":false,"send_date":"...","mes":"..."}
   * ```
   * **追加，不改寫既有內容。** 這跟 `stampChatSessionId` 不一樣——那一條動的是第一行
   * （標頭），必須重寫整個檔案；這一條只往尾巴接。
   *
   * @param character - 角色 id。
   * @param chat - 對話名。
   * @param messages - `[{ name, isUser, text }]`；空字串的訊息會被跳過。
   * @returns 實際寫進去的則數。
   */
  async appendChatMessages(character, chat, messages) {
    const who = requireId(character, '角色 id')
    const title = segmentFromName(chat)
    if (title === '') throw new Error('對話名稱不可為空')
    const path = this.resolveWithin('chats', who, `${title}.jsonl`)

    // 對話檔一定要先存在——不存在就「追加」等於憑空生一份沒有標頭的檔案，
    // 那不是有效的酒館聊天檔。
    if ((await stat(path).catch(() => undefined)) === undefined) {
      throw new Error(`找不到這份對話：chats/${who}/${title}.jsonl`)
    }
    ...
    for (const message of Array.isArray(messages) ? messages : []) {
      ...
      if (text === '') continue
      const name = typeof message?.name === 'string' && message.name !== '' ? message.name : 'unused'
      // 思考（reasoning）放在 `extra.reasoning`：SillyTavern 的訊息本來就有這個
      // 自由欄位，所以帶著走的檔案裡思考不會丟，別的軟體也讀得懂這一則訊息。
      const reasoning = typeof message?.reasoning === 'string' ? message.reasoning : ''
      const record = {
        name,
        is_user: message?.isUser === true,
        is_system: message?.isSystem === true,
        send_date: typeof message?.sendDate === 'string' && message.sendDate !== '' ? message.sendDate : nowIso(),
        mes: text,
      }
   ...
   * 讀一份對話的訊息（不含標頭行）。
   *
   * @returns `[{ name, isUser, isSystem, sendDate, text, reasoning }]`；讀不到時回空陣列。
   */
  async readChatMessages(character, chat) {
    const who = requireId(character, '角色 id')
    const title = segmentFromName(chat)
    if (title === '') return []
    let raw
    try {
      raw = await readFile(this.resolveWithin('chats', who, `${title}.jsonl`), 'utf8')
    } catch {
      return []
    }
    const lines = raw.split('\n')
    const messages = []
    // 第 0 行是標頭，從第 1 行開始才是訊息。
    for (let index = 1; index < lines.length; index += 1) {
      ...
      try {
        ...
        // 壞掉的一行就跳過——**不要讓一行壞資料讓整份對話讀不出來**。
        continue
      }
      if (parsed === null || typeof parsed !== 'object') continue
      const extra = parsed.extra !== null && typeof parsed.extra === 'object' ? parsed.extra : null
      messages.push({
        name: typeof parsed.name === 'string' ? parsed.name : 'unused',
        isUser: parsed.is_user === true,
        isSystem: parsed.is_system === true,
        sendDate: typeof parsed.send_date === 'string' ? parsed.send_date : '',
        text: typeof parsed.mes === 'string' ? parsed.mes : '',
        // 思考（如果那一則有）。沒有就是空字串，呼叫端不必再判一次 null。
        reasoning: extra !== null && typeof extra.reasoning === 'string' ? extra.reasoning : '',
      })
    }
    return messages
  }
   ...
   * @param record - `{ character, chat }`；character 是角色 id、chat 是對話名。
  ...
  async rebuildSessionBindings() {
    ...
    for (const chat of chats) {
      const sessionId = await this.readChatSessionId(chat.character, chat.name)
      if (sessionId === null) continue
      await this.bindSession(sessionId, { character: chat.character, chat: chat.name })
      rebuilt += 1
    }
    return rebuilt
  }
  ...
  async readChatSessionId(character, chat) {
    ...
    try {
      const handle = await open(this.resolveWithin('chats', who, `${title}.jsonl`), 'r')
      try {
