...
export function defaultRoom(name) {
  return {
     ...
     * `position` 再蓋過它（由窄到寬：**這一間房對那一本書的指定** → 書 → 房 → 酒館）。
     */
    ...
    worldbookOverrides: {},
    /**
     * 這一間房對**個別條目** `order`（優先序）的覆寫（2.6.71）。
     *
     * ⚠️ 形狀 `{ "<書的 id>": { "<條目的鍵>": { order } } }`，**沒列到的條目
     * 照書自己的 `order`**——所以既有房間一字不差。條目的鍵見 `entryKeyOf`
     * （`uid` 優先，沒有 `uid` 才用 `#<索引>`）。
     *
     * ⚠️ **與 `worldbookOverrides` 分開**是刻意的：「這一場用不用這本書、放哪裡」
     * 與「這一場裡某一條排多前面」是兩件事，混在同一個物件裡會讓
     * 「整份覆寫取代」的 patch 很難寫對（漏帶一個鍵就會清掉別的設定）。
     *
     * ⚠️ **書的檔案一個字都不會被改**——這正是使用者要的（「不包括內容，
     * 只是修改位置以及優先序」）。
     */
    worldbookEntryOverrides: {},
    createdAt: nowIso(),
  }
}
...
export class TavernWorkspace {
   ...
   *
   * ⚠️ **三態**（2.6.71）：`null`／`''` ＝ **把那個鍵刪掉**（＝回到上一層：
   * 這一間房的預設／酒館的預設／`in-chat`），三個位置名之一 ＝ 寫進去。
   * 刪得掉是必要的——在那之前畫面上寫著「要回到跟著酒館預設，請到原始 JSON
   * 把 position 刪掉」，那是一個**叫使用者去用別的工具**的設定。
   *
   * ⚠️ 位置存**我們的名字**（`'system-after'` 這種字串），不是 ST 的數字
   * ——但 `positionOf()` 兩種都讀得懂，所以舊檔不會壞。
   *
   * @param id - 世界書 id。
   * @param where - `WORLDBOOK_POSITIONS` 之一，或 `null`／`''`（＝清掉）。
   * @returns 寫進去的值（清掉時是 `null`）。
   */
   ...
   * 放在哪」——而那要四層一起算（房 → 書 → 房預設 → 酒館）。用酒館那一支的結果
   * 會顯示錯的位置，而那是**安靜的**：使用者看到一個位置，實際送去的是另一個。
   *
   * @param character - 角色 id。
   * @param room - 房間 id。
   * @returns `{ room, tavern, overrides, books }`——`books` 每一項有
   *   `position`（這一間房算完的）、`ownPosition`（**書自己的**，沒有就空字串）、
   *   `explicit`（書自己有沒有指定）。
   */
  async roomWorldbookPositions(character, room) {
    ...
    for (const one of await this.listWorldbooks()) {
      ...
      out.push({
         ...
         * 2.6.69 起這一間的指定排在**最前面**（見 `positionOf`）。
         */
        position: positionOf(applyBookOverride({ id: one.id, data }, overrides)?.data, roomFallback, tavernFallback),
        /**
         * ⚠️ **書自己指定的那一個值**（不是算完的）。
         *
         * 房間那一頁的「放在哪」下拉只有兩種值：這一間房指定的、或**空字串**
         * （＝不指定）。空格子要老實寫出「不指定的話會用什麼」，而那取決於
         * **書自己有沒有指定**——所以它必須是書檔案裡的原值。
         *
         * 用 `position` 代替它在這一間房指定過的時候會**說謊**
         * （那時 `position` 是房間的值，不是書的）。
         */
        ownPosition: normalizePosition(data?.position),
        explicit: normalizePosition(data?.position) !== '',
        /**
    ...
    return {
      ...
      overrides: overrides,
      /**
       * ⚠️ **條目層的覆寫也要回去**：房間那一頁送 patch 時是**整份取代**，
       * 它得先知道現在有哪些（不然調一條就會把別的清掉）。
       */
      entryOverrides:
        roomData?.worldbookEntryOverrides !== null &&
        typeof roomData?.worldbookEntryOverrides === 'object' &&
        Array.isArray(roomData.worldbookEntryOverrides) === false
          ? roomData.worldbookEntryOverrides
          : {},
      books: out,
    }
  }
  ...
  /**
   * **這一間房裡**，某一本世界書的每一條條目（含它的優先序）。
   *
   * ⚠️ **為什麼要有這一支**（而不是叫客戶端自己 `worldbook.read` 再算）：
   *   1. **條目的鍵由宿主半算**（`entryKeyOf`：`uid` 優先、沒有才 `#<索引>`）。
   *      客戶端要送出覆寫就得知道每一條的鍵——兩邊各算一份就是「同一個東西
   *      兩個來源」，走散時症狀是**安靜的**（覆寫存到不存在的鍵上、永遠不生效）。
   *   2. 「**這一間房**的優先序是多少」是四層算完的結果（房 → 書 → ST 預設），
   *      而那只有宿主半算得對。
   *
   * 回傳的每一條：`order` ＝ 這一間房算完的，`ownOrder` ＝ **書自己的**
   * （房間那一頁要靠它做「還原」），`overridden` ＝ 這一間房有沒有指定過。
   *
   * ⚠️ **內容唯讀**：這一支順便回 `content`（房間那一頁的「▸ 內容」要畫它），
   * 但**沒有任何寫入條目的路**——房間不改書的內容，那是使用者一再強調的。
   *
   * @param character - 角色 id。
   * @param room - 房間 id。
   * @param book - 世界書 id。
   * @returns `{ book, position, entries }`。
   */
  async roomEntries(character, room, book) {
    const id = await this.resolveRoom(character, room)
    const roomData = await this.readRoom(character, id)
    const key = typeof book === 'string' ? book : ''
    const listed = await this.listWorldbooks()
    if (listed.some((one) => one.id === key) === false) throw new Error(`找不到這本世界書：${key}`)
    let data = null
    try {
      data = JSON.parse(await readFile(this.resolveWithin('worldbooks', `${key}.json`), 'utf8'))
    } catch {
      throw new Error(`這本世界書讀不出來（不是合法的 JSON）：${key}`)
    }
    const entryOverrides =
      ...
      Array.isArray(roomData.worldbookEntryOverrides) === false
        ? roomData.worldbookEntryOverrides
        : {}
    const bag = entryOverridesOf(entryOverrides, key)
    const list = normalizeEntries(data)
    const entries = list.map((entry, index) => {
      const entryKey = entryKeyOf(entry, index)
      const mine = bag[entryKey]
      const ownOrder = typeof entry.order === 'number' ? entry.order : 100
      const fromRoom = mine !== null && typeof mine === 'object' ? normalizeOrder(mine.order) : null
      return {
        key: entryKey,
        ...
        /** 這一間房算完的優先序（房間的覆寫 → 書自己的 → 100）。 */
        order: fromRoom === null ? ownOrder : fromRoom,
        /** 書自己的值——房間那一頁的「還原」要打回它。 */
        ownOrder: ownOrder,
        overridden: fromRoom !== null,
      }
    })
    return {
      book: key,
      // 這一本書在這一間房的位置（房間 → 書 → 房預設 → 酒館預設）。
      position: positionOf(data, roomData?.worldbookPosition, (await this.readSettings()).worldbookPosition),
      entries: entries,
    }
  }
  ...
  async writeRoom(character, room, patch) {
    ...
    /**
     * 這一間房對**個別條目** `order` 的覆寫（2.6.71）。
     *
     * ⚠️ **兩層 id 都要驗**（書的 id 與條目的鍵）：不驗的話打錯一個字就會留下
     * 一筆永遠不會生效的設定，而使用者以為自己調好了——那是安靜的失敗。
     * 書不存在／條目不存在／`order` 不是整數，三種都收進 `dropped` 一起回報。
     *
     * ⚠️ **`order` 與書自己的值相同時照樣存**：那在排序上等價，但「這一間房
     * 明確指定過」這件事本身有意義（書以後改了，這一間不會跟著跑）——
     * 而客戶端的「還原」送的是 `null`（刪掉那個鍵），不是送原值。
     */
    if (patch?.worldbookEntryOverrides !== undefined) {
      const wanted = patch.worldbookEntryOverrides
      if (wanted === null) {
        next.worldbookEntryOverrides = {}
      } else if (typeof wanted !== 'object' || Array.isArray(wanted)) {
        roomDropped.push('worldbookEntryOverrides（要是一個物件）')
      } else {
        const listed = await this.listWorldbooks()
        const known = new Set(listed.map((one) => one.id))
        const out = {}
        for (const [bookId, entries] of Object.entries(wanted)) {
          if (known.has(bookId) === false) {
            roomDropped.push(`worldbookEntryOverrides（沒有這本世界書：${bookId}）`)
            continue
          }
          if (entries === null || entries === undefined) continue
          if (typeof entries !== 'object' || Array.isArray(entries)) {
            roomDropped.push(`worldbookEntryOverrides（${bookId} 要是一個物件）`)
            continue
          }
          // 條目的鍵要真的在那一本書裡——**讀不到書就不驗**（書壞掉不該讓房間寫不進去）。
          let keys = null
          try {
            const data = JSON.parse(await readFile(this.resolveWithin('worldbooks', `${bookId}.json`), 'utf8'))
            keys = new Set(normalizeEntries(data).map((entry, index) => entryKeyOf(entry, index)))
          } catch {
            keys = null
          }
          const bag = {}
          for (const [entryKey, value] of Object.entries(entries)) {
            if (keys !== null && keys.has(entryKey) === false) {
              roomDropped.push(`worldbookEntryOverrides（${bookId} 裡沒有這一條：${entryKey}）`)
              continue
            }
            if (value === null || value === undefined) continue
            if (typeof value !== 'object' || Array.isArray(value)) {
              roomDropped.push(`worldbookEntryOverrides（${bookId}／${entryKey} 要是一個物件）`)
              continue
            }
            if (value.order === null || value.order === undefined || value.order === '') continue
            const order = normalizeOrder(value.order)
            if (order === null) {
              roomDropped.push(
                `worldbookEntryOverrides（${bookId}／${entryKey} 的優先序要是 ${ORDER_LIMITS.min}～${ORDER_LIMITS.max} 的整數：${JSON.stringify(value.order)}）`,
              )
              continue
            }
            bag[entryKey] = { order }
          }
          if (Object.keys(bag).length > 0) out[bookId] = bag
        }
        next.worldbookEntryOverrides = out
      }
    }
    next.updatedAt = nowIso()
