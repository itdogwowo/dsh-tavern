...
/**
 * 把卡片資料**換成**這一版（寫回用的那一支）。
 *
 * ⚠️ 跟 {@link writeCardIntoPng} 的差別只有一個，但那一個很致命：**舊的卡片區塊會被拿掉**。
 * `writeCardIntoPng` 是「接上去」（來源圖本來沒有卡片資料，例如 `build-sample.mjs`），
 * 而讀取端是 `ccv3` 優先——所以對一張**已經有 `ccv3` 的 PNG**再 `writeCardIntoPng(…, {keyword:'chara'})`，
 * 讀回來的仍然是**舊的 ccv3**：使用者在面板上改了卡，存檔看起來成功，實際上沒生效。
 *
 * 這裡的規矩：走一次 chunk，**跳過所有卡片區塊**（`ccv3`／`chara`…），其餘原封不動，
 * 最後在 `IEND` 前插入一個新的。`IDAT` 不重新編碼，所以畫質零損失。
 *
 * @param bytes - 原始 PNG 檔內容
 * @param card - 卡片物件（會被 `JSON.stringify`）
 * @param options - `{ keyword }`；預設沿用讀到的那一個（`readCardFromPng` 回報的）
 * @returns 新的 PNG buffer
 * @throws 不是 PNG、或結構壞到找不到 `IEND` 時丟錯
 */
export function replaceCardInPng(bytes, card, options = {}) {
  if (!isPng(bytes)) throw new Error('這不是 PNG 檔（檔頭不對）')
  const keyword = options.keyword === undefined ? 'chara' : String(options.keyword)
  const value = Buffer.from(JSON.stringify(card), 'utf8').toString('base64')

  const keep = [bytes.subarray(0, SIGNATURE.length)]
  let offset = SIGNATURE.length
  let scanned = 0
  let dropped = 0
  let found = false
  while (offset + 8 <= bytes.length) {
    ...
    if (type === 'IEND') {
      found = true
      keep.push(textChunk(keyword, value))
    ...
    if (type === 'tEXt') {
      ...
      const separator = data.indexOf(0x00)
      const existing = separator > 0 ? data.toString('latin1', 0, separator) : ''
      if (CARD_KEYWORDS.includes(existing)) {
        // 這一個是舊的卡片資料 → 丟掉（新的等下會插進去）。
        dropped += 1
        offset = next
        continue
      }
    }
    keep.push(bytes.subarray(offset, next))
    offset = next
  }
  if (!found) throw new Error('PNG 少了 IEND 區塊（檔案不完整）')
  const out = Buffer.concat(keep)
  // 沒有任何舊卡片區塊時，`replaceCardInPng` 跟 `writeCardIntoPng` 應該等價——
  // 這一條讓「寫回」與「第一次接上去」走同一個結果，不必分兩套。
  return dropped === 0 && options.keyword === undefined ? writeCardIntoPng(bytes, card, options) : out
}
