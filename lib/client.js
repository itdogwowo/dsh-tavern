window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * ⚠️ **這兩個模組層級的狀態宣告在最前面是刻意的，不要往下搬。**
     *
     * `MapWorldbooks`（檔案中段）與「跳到某一本書」都會讀它們，而
     * `var` 的提升**只提升宣告、不提升值**——搬到它們的使用點之後，
     * 那些地方讀到的是 `undefined`。
     *
     * 這一型踩過兩次：2.6.57 的 `PARSE_DEFAULT_CONFIG`（症狀是整頁白屏）、
     * 2.6.65 的 `pendingBookOpen`（症狀是 `open(undefined)` 靜靜地什麼都不做）。
     */

    /** 「跳到某一本書」的交接格：房間那一頁放 id，`MapWorldbooks` 掛載時撿走。 */
    var pendingBookOpen = ''
...
try {
  ...
  PRIMITIVES = null
     ...
     */
    var CLIENT_BUILD = 'tavern-client-2.6.66'
    ...
    var MAP_CSS = [
      ...
      // 房間那一頁「📖 藏書」的一列：開關／書名／位置／標籤／內容 擠在同一條。
      // 用 `flex-wrap` 而不是硬擠——面板變窄時它自己折行，不會把書名壓成一個字。
      '.dsh-tv-bookHead{display:flex;flex-wrap:wrap;align-items:center;gap:8px}',
      // ⚠️ **書名本身就是「去改它」的入口**（2.6.66，使用者的要求）。
      //
      // 之前是兩顆長得一樣的按鈕（「看內容」「去改它」），而「看內容」當場就把內文
      // 攤開——其中一顆先把另一顆的事做掉一半，使用者看不出差別
      // （回報：「看內容其實你的設計方法是將他接的起來了」）。現在一個動作一顆：
      //   書名   ＝ **去別的地方改**（換到 📖 藏書 分區並打開這一本）
      //   「內容」＝ **就地展開**（唯讀，不離開這一頁）
      //
      // 用 `<button>` 而不是 `<a>`：沒有網址可去，只是換一個分區，
      // 而 `<a href="#">` 會讓瀏覽器換頁（也讓「上一頁」多一筆垃圾）。
      '.dsh-tv-bookLink{flex:1;min-width:80px;border:none;background:none;padding:0;font:inherit;font-size:12.5px;',
      'color:var(--dsh-tv-accent);text-align:left;cursor:pointer;text-decoration:underline;text-underline-offset:2px;',
      'overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
      '.dsh-tv-bookLink:hover{color:var(--dsh-tv-text-1)}',
      // 擠在一列裡的位置選單。
      //
      ...
      // 「內容」展開的書（唯讀預覽）。
      '.dsh-tv-bookPeek{margin-top:2px;padding:6px 8px;',
    ...
    function MapWorldbooks(props) {
      ...
      /** 改一本書的位置（**只送那一個欄位**，宿主半只改它）。 */
      function setBookPosition(id, where) {
        state.error = ''
        state.message = ''
        if (where === '') {
          // 「跟著酒館預設」＝把書裡那個欄位拿掉。目前沒有「刪欄位」的 op，
          // 所以先講清楚——不要假裝成功（那是這一組欄位最糟的失敗方式）。
          state.error = '目前只能改成三個位置之一；要回到「跟著酒館預設」，請到原始 JSON 把 position 刪掉。'
          render()
          return
        }
        rpc('worldbook.position', { book: id, where: where })
          .then(function () {
            state.message = '已把這一本書放到「' + WORLDBOOK_POSITION_INFO[where].label + '」'
            render()
            return loadList()
          })
          .catch(function (error) {
      ...
      React.useEffect(function () {
        loadList()
        // ⚠️ 房間那一頁按「到 📖 藏書 打開」時，會把書 id 放在 `pendingBookOpen`。
        //    在**掛載之後**撿走（渲染期改狀態會被 React 警告）。
        var wanted = takePendingBookOpen()
        if (wanted !== '') open(wanted)
        /**
         * ⚠️ **掛載時撿一次不夠。**
         *
         * 這一頁只在 `currentZone === 'books'` 時才掛著，所以「人已經停在 📖 藏書
         * 分區，卻從房間那一頁按了另一本書」的時候它**不會重新掛載**——只靠掛載
         * effect 的話，那一下會靜靜地什麼都不做（使用者回報：「去改它沒有跳過去」）。
         * 所以另外訂閱 `bookOpenChannel`：換書與換分區是兩件事，各有一條通道。
         */
        return bookOpenChannel.watch(function () {
          var next = takePendingBookOpen()
          if (next !== '') open(next)
        })
      }, [])
          ...
          state.selected === null
            ...
            : React.createElement(
                ...
                React.createElement(MapSelect, {
                  ...
                  onChange: function (value) {
                    setBookPosition(state.selected, value)
                  },
                }),
                React.createElement(
                  'div',
    ...
    /**
     * 「有人要求打開某一本世界書」——**只給 `MapWorldbooks` 一個人聽**。
     *
     * 為什麼不搭 `refreshChannel`：那一條的意思是「清單可能過期了，重讀」，
     * 而它的訂閱者（`MapWorldbooks` 那一條 `useRefreshVersion`）會**重讀清單**
     * ——對「打開某一本」來說那是多餘的一趟，而且它只在掛載時註冊一次，
     * 拿它來傳「要開哪一本」會讓語意變得不能預期。
     *
     * 為什麼需要它：`MapWorldbooks` 只在 `currentZone === 'books'` 時才掛著。
     * 人**已經停在 📖 藏書**、又從房間那一頁按了另一本書時，它不會重新掛載，
     * 掛載 effect 就不會跑——那一下看起來像壞了（實際回報過）。
     */
    var bookOpenChannel = makeVersionChannel()
     ...
     * 撿走「請你打開這一本」（沒有就回空字串）。
     *
     * ⚠️ 抽成模組層級的函式是為了**驗得到**：交接格是模組層級的狀態，
     * 而「撿走之後就清掉」正是它唯一的行為（忘了清 ＝ 下一次重畫又跳一次）。
     */
    function takePendingBookOpen() {
      if (pendingBookOpen === '') return ''
      var wanted = pendingBookOpen
      pendingBookOpen = ''
      return wanted
    }

    /**
     * 到「📖 藏書」分區打開某一本書（房間那一頁的書名按下去就是這支）。
     *
     * ⚠️ **三件事，缺一那一下就會「沒有跳過去」**（實際回報過）：
     *   1. `pendingBookOpen`：要打開**哪一本**——交給 `MapWorldbooks` 撿。
     *   2. `currentZone = 'books'` ＋ `refreshChannel.bump()`：換分區是模組層級的
     *      狀態，設定頁已經停在這一頁時不會自己重畫（跟側邊欄那顆 ＋ 同一個坑）。
     *   3. `selectPanel(TAVERN_SETTINGS_KEY)`：**主面板也要指過去**。
     *      房間那一頁是另一張面板（`TAVERN_CHAT_KEY`），只換分區的話使用者
     *      還停在房間裡——畫面完全不動，看起來就像按鈕壞了。
     */
    function jumpToBook(bookId) {
      pendingBookOpen = bookId
      currentZone = 'books'
      bookOpenChannel.bump()
      refreshChannel.bump()
      selectPanel(TAVERN_SETTINGS_KEY)
    }
     ...
     *   > 房間的時候其微調可以自己在整理兩層，所以要加一個新標籤
     *
     * 三件事：**這一間房的預設位置**、**每一本書的開／關**、**每一本書的位置**。
     * 而三層優先序（書 → 房 → 酒館）由宿主半算，這一頁只顯示與送 patch。
     *
     * ⚠️ **`explicit`（書自己指定位置）的那一列要標出來**：它的位置**不受
     * 這一頁影響**，不講的話使用者會改了沒反應、然後以為壞掉。
     */
    function RoomBooksPane(props) {
      ...
      var ref = React.useRef(null)
      if (ref.current === null) {
        ref.current = { info: null, error: '', notice: '', busy: false, loaded: false, key: '', content: null, contentBusy: '' }
      }
      var state = ref.current
      ...
      /** 讀一本書的內容（按「看內容」才讀，讀到就記在 state 裡）。 */
      var loadContent = function (bookId) {
        ...
        render()
        return rpc('worldbook.read', { book: bookId })
          .then(function (data) {
            var entries = []
            var list =
              data !== null && data !== undefined && Array.isArray(data.entries)
                ? data.entries
                : data !== null && data !== undefined && data.entries !== null && typeof data.entries === 'object'
            ...
            for (var one of list) {
              ...
              entries.push({
                label:
                  typeof one.comment === 'string' && one.comment.trim() !== ''
                    ? one.comment.trim()
                    : typeof one.name === 'string' && one.name.trim() !== ''
                      ? one.name.trim()
                      : '（沒有標題）',
                content: typeof one.content === 'string' ? one.content : '',
                constant: one.constant === true,
                disabled: one.disable === true,
              })
            }
            state.contentBusy = ''
            state.content = { id: bookId, entries: entries }
            render()
          })
          .catch(function (error) {
            state.contentBusy = ''
      ...
      /**
       * 這一本書在這一間房的位置那一格要顯示什麼（三態）。
       *
       * ⚠️ **唯讀**：這裡只顯示「這一間房怎麼讀它」，改內容一律回 📖 藏書。
       * 三態的來源依序是：書自己指定的 → 這一間房的覆寫 → 空字串（＝聽這一間房的預設）。
       */
      var roomBookPositionValueOf = function (one) {
        if (one.explicit === true) return one.position
        var overrides =
          state.info !== null && state.info !== undefined && state.info.overrides !== undefined ? state.info.overrides : {}
        var mine = overrides[one.id]
        if (mine !== null && mine !== undefined && typeof mine.position === 'string') return mine.position
        return ''
      }

      /** 送一次覆寫 patch（只動那一本書）。 */
      var patchBook = function (bookId, patch) {
        ...
        if (patch === null) delete next[bookId]
        else next[bookId] = patch
        rpc('room.write', {
      ...
      kids.push(
        React.createElement(
          ...
          books.map(function (one) {
            ...
            head.push(
              React.createElement(MapToggle, {
                ...
                onChange: function (next) {
                  ...
                  patchBook(one.id, next === false ? { enabled: false } : null)
                },
              }),
            )
            /**
             * ⚠️ **書名本身就是那顆「去改它」**（使用者：「看內容其實你的設計方法是
             * 將他接的起來了，所以按鈕可能要再設計一下」）。
             *
             * 原本一列有兩顆長得一樣的按鈕（「看內容」「去改它」），而「看內容」
             * 當場就把內文攤開——等於其中一顆先把另一顆的事做掉一半，使用者分不出
             * 差別。現在**一個動作一個入口**：
             *   書名   ＝ 去別的地方改（這裡是唯讀的，要改內容得去 📖 藏書）
             *   「內容」＝ 就地展開（唯讀，不離開這一頁）
             * 而且「名字可以點」是清單的既有直覺，比多一顆按鈕好記。
             */
            head.push(
              React.createElement(
                'button',
                {
                  key: 'name',
                  type: 'button',
                  className: 'dsh-tv-bookLink',
                  title: '到酒館的「📖 藏書」分區打開這一本（在那裡改內容）',
                  onClick: function () {
                    jumpToBook(one.id)
                  },
                },
            ...
            // 位置：三態（照這一間房的預設／三個位置）。書自己指定的那一本要標出來。
            head.push(
              React.createElement(
                'select',
                {
                  ...
                  title: one.id + '：這一本書在這一間房放在哪',
                  onChange: function (event) {
                    var value = event.target.value
                    patchBook(one.id, value === '' ? null : { position: value })
                  },
                },
                [
                  { value: '', label: '聽這一間房的' },
            ...
            if (one.explicit === true) {
              head.push(
                React.createElement(
                  'span',
                  { className: 'dsh-tv-posTag', key: 'explicit', title: '這一本書自己在「📖 藏書」指定了位置' },
                  '書自己指定',
                ),
              )
            }
             ...
             * 「內容」＝**就地展開**（唯讀）。
             *
             * 以前它叫「看內容」而旁邊還有一顆「去改它」——兩顆都是按鈕、都跟這一本
             * 書有關，看不出哪一顆會離開這一頁。現在這一顆只做一件事（展開／收起），
             * 而且用 ▸／▾ 表明它是**摺疊**而不是動作。
             */
            ...
            head.push(
              React.createElement(
                'button',
                {
                  ...
                  title: opened ? '收起這一本的內容' : '在這裡展開這一本的內容（唯讀）',
                  'aria-expanded': opened ? 'true' : 'false',
                  onClick: function () {
                    loadContent(one.id)
                  },
                },
                state.contentBusy === one.id ? '讀取中…' : opened ? '▾ 內容' : '▸ 內容',
              ),
            )
            // 展開的內容（唯讀）掛在那一列下面。
            var peek = null
            if (opened) {
              ...
              peek = React.createElement(
                'div',
                { className: 'dsh-tv-bookPeek' },
                entries.length === 0
            ...
            return React.createElement(
              'div',
              { key: one.id, className: 'dsh-tv-entry' },
              React.createElement('div', { className: 'dsh-tv-bookHead' }, head),
              peek,
            )
          }),
        ),
      ...
      kids.push(
        React.createElement(
          ...
          '⚠️ 這裡只調「這一場怎麼讀」——**書的內容一個字都不會被改**。' +
            '要改內容：**點書名**跳到「📖 藏書」（那裡才是編輯器）；只想先看一眼，按「▸ 內容」就地展開。' +
            '標著「書自己指定」的那一本，位置由它自己決定。',
        ),
      )
      if (state.error !== '') {
     ...
     *
     * ⚠️ **宣告搬到最上面了**（跟 `pendingBookOpen` 一起，在 `CHAT_TABS` 之前）：
        ...
        var panes =
          currentChatTab === 'room'
            ? [
                 ...
                 * 三個位置名之一 ＝ 這一間房自己的選擇。
                 * ⚠️ 每一本書自己的 `position` 還是**最優先**（書 → 房 → 酒館），
                 * 所以這一格是「這一間房的預設」，不是「強制覆蓋全部」。
                 */
                React.createElement(MapSelect, {
                  label: '這一間房的「藏書放哪裡」',
                  value:
            ...
            : currentChatTab === 'books'
              ? [
                   ...
                   * 這一間房最後會怎樣**（三層算完的結果），而每一列可以：
                   *   - 打開／關掉（這一間房不要這本書）
                   *   - 改位置（蓋過酒館那一層；但**蓋不過書自己指定的**）
                   *
    ...
    module.exports = {
       ...
       * 房間那一頁 →「📖 藏書」的跳轉（測試用）。
       *
       * ⚠️ 這一支是**修過 bug 的**那一段（「去改它」按了沒反應）：它必須做三件事
       * ——放書 id、換分區、換主面板。所以它單獨匯出，讓測試能真的叫它一次並看
       * `currentZone`／`shownPanel`／那格交接值有沒有都對。
       */
      __jumpToBook: jumpToBook,
      __pendingBookOpen: function () {
        return pendingBookOpen
      },
      __takePendingBookOpen: takePendingBookOpen,
      __bookOpenChannel: bookOpenChannel,
      /**
       * 驅動對話頁的載入（測試用）。
       *
