window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *
     * ⚠️ **2.6.72 把 `pendingBookOpen` 加回來**（使用者：「▸ 條目 這個按鈕其實
     * 可以是跳轉去修改」）——所以它**又**回到這個最前面的區塊，理由同上面那三次。
     */

    /**
     * 「跳到某一本書」的交接格：房間那一頁放 id，`MapWorldbooks` 掛載時撿走。
     *
     * ⚠️ **空字串＝沒有待辦**（不是 `null`）：這一格是模組層級的字串，而
     * `open(undefined)` 的症狀是**靜靜地什麼都不做**（2.6.65 真的踩過）。
     */
    var pendingBookOpen = ''
...
try {
  ...
  PRIMITIVES = null
     ...
     */
    var CLIENT_BUILD = 'tavern-client-2.6.72'
    ...
    var MAP_CSS = [
      ...
      /**
       * ⚠️ **整條列都可以摺疊**（2.6.72）——所以這一列現在是**可按的**，
       * 要給得出「它可以按」的樣子（游標、hover 底色、鍵盤 focus 的外框）。
       *
       * ⚠️ 控制項自己會 `stopPropagation`（見 `stopRowToggle`），
       * 所以按開關或位置選單不會順手把這一列展開。
       */
      '.dsh-tv-bookRowClick{cursor:pointer;border-radius:var(--dsh-tv-radius-sm)}',
      '.dsh-tv-bookRowClick:hover{background:var(--dsh-tv-surface-1)}',
      '.dsh-tv-bookRowClick:focus-visible{outline:2px solid var(--dsh-tv-accent);outline-offset:1px}',
      /**
       * 折疊箭頭（每一列的最左邊；`▸` 轉 90° 變 `▾`）。
       *
       * ⚠️ 用 `transform` 而不是換字：換字會讓那一格的寬度跟著變，
       * 整排就會在展開／收起時左右跳（這個 repo 對「按鈕錯位」已經吃過一次）。
       */
      '.dsh-tv-peekArrow{flex:none;width:10px;font-size:10px;line-height:1;color:var(--dsh-tv-text-3);',
      'transition:transform .12s ease}',
      '.dsh-tv-peekArrowOpen{transform:rotate(90deg)}',
      /**
       * 包住「列上的控制項」的那一層（開關、位置選單）——它只負責吃掉冒泡事件。
       *
       * ⚠️ `display:contents` 讓這一層**不參與排版**（它不產生盒子，裡面的東西
       * 直接當那一列的 flex item）——加這一層前後版面完全一樣。
       * 沒有它就得把 `onClick` 傳進 `MapToggle`／`select` 裡面，那是把「摺疊」
       * 這件事塞進兩個共用元件；包一層便宜得多。
       */
      '.dsh-tv-rowCtl{display:contents}',
      /**
       * 「▸ 內容」展開的書（唯讀預覽）。
       *
    ...
    function MapWorldbooks(props) {
      ...
      React.useEffect(function () {
        loadList()
        /**
         * ⚠️ 房間那一頁按「✎ 修改」時，會把書 id 放在 `pendingBookOpen`。
         ...
         * effect 的話，那一下會靜靜地什麼都不做（2.6.66 實際回報過：「去改它沒有
         * 跳過去」）。所以另外訂閱 `bookOpenChannel`：換書與換分區是兩件事。
         */
     ...
     * 「**請打開這一本書**」的通道（2.6.72）。
     *
     * ⚠️ **不能借用 `refreshChannel`**：那一條的語意是「內容可能變了，重讀」，
     * 而「換一本正在編輯的書」不是重讀——`MapWorldbooks` 對兩者的反應不一樣
     * （重讀會蓋掉使用者正在編輯的草稿）。舊版（2.6.66）就是為了這個理由分成兩條。
     *
     * ⚠️ 為什麼需要它：`MapWorldbooks` 只在 `currentZone === 'books'` 時才掛著。
     * 人**已經停在 📖 藏書**、又從房間那一頁按了「✎ 修改」時，它不會重新掛載，
     * 掛載 effect 就不會跑——那一下看起來像壞了（2.6.66 實際回報過）。
     */
    ...
    /**
     * 到「📖 藏書」分區打開某一本書（房間那一頁的「✎ 修改」按下去就是這支）。
     *
     * 使用者（2.6.72）：
     *   > ▸ 條目，這個按鈕其實可以是跳轉去修改
     *
     * ⚠️ **三件事，缺一那一下就會「沒有跳過去」**（2.6.66 實際回報過）：
    ...
    function RoomBooksPane(props) {
      ...
      /**
       * 攔掉冒泡：**整條列都可以摺疊**（2.6.72），所以列裡面的每一個控制項
       * 都要自己把 click 吃掉——不然「按開關」會順手把這一列展開／收起。
       *
       * ⚠️ 抽成一個具名函式而不是到處寫 inline：漏掉一個地方的症状是
       * 「按那個東西畫面會多跳一下」，而那很容易被當成「本來就這樣」。
       */
      var stopRowToggle = function (event) {
      ...
      kids.push(
        React.createElement(
          ...
          books.map(function (one) {
            var head = []
            /**
             * ⚠️ **展開狀態要在這一列的最前面算**（`head` 的每一項都要看它）：
             * 折疊箭頭、`aria-expanded`、以及展開的那一塊。
             */
            var opened = state.content !== null && state.content !== undefined && state.content.id === one.id

            /**
             * **折疊箭頭**（整條列都可以摺疊，這是它的視覺提示）。
             *
             * 使用者（2.6.72）：
             *   > 然後摺疊功能改為整條都可以摺疊
             *
             * ⚠️ 放在**最左邊**：每一列都有它，所以不會有「有的列有、有的列沒有」
             * 造成的左右跳動（那是「書自訂」標籤上一輪被打回來的原因）。
             */
            head.push(
              React.createElement(
                'span',
                { className: 'dsh-tv-peekArrow' + (opened ? ' dsh-tv-peekArrowOpen' : ''), key: 'arrow', 'aria-hidden': 'true' },
                '▸',
              ),
            )
            // 開／關（這一間房不要這本書）。
            // ⚠️ 包一層攔掉冒泡：這一顆是**動作**，按它不該順手把整列展開／收起。
            head.push(
              React.createElement(
                'span',
                { key: 'onWrap', className: 'dsh-tv-rowCtl', onClick: stopRowToggle },
                React.createElement(MapToggle, {
                  key: 'on',
                  label: '用這本書',
             ...
             */
            head.push(
              // ⚠️ 包一層攔掉冒泡（見 `stopRowToggle`）：選位置不該把整列展開／收起。
              React.createElement(
                'span',
                { key: 'posWrap', className: 'dsh-tv-rowCtl', onClick: stopRowToggle },
                React.createElement(
                  'select',
                  {
                    key: 'pos',
            ...
            /**
             * ⚠️ **2.6.72：這一顆按鈕改成「跳到 📖 藏書 去改」**，展開改由**整條列**負責。
             *
             * 使用者：
             *   > ▸ 條目，這個按鈕其實可以是跳轉去修改
             *   > 然後摺疊功能改為整條都可以摺疊
             *
             * 兩件事分開之後，這一頁的每個入口只做一件事：
             *   - **整條列**（點了就展開／收起）＝ 看這一本書的條目，順便調**這一間房**
             *     的優先序（那一頁的內容仍然是唯讀的）
             *   - **這顆按鈕**＝ 離開這一頁，到酒館的「📖 藏書」打開這一本（在那裡改
             *     標題、關鍵字、`order`、**內容**——那是書的檔案，只有那一頁會改它）
             *
             * ⚠️ **這就是 2.6.66 被 2.6.68 拿掉的那一條路**（`jumpToBook` 那一組），
             * 現在照使用者新的要求加回來——所以實作直接沿用舊的（三件事缺一不可，
             * 見 `jumpToBook` 的註解）。
             */
            head.push(
              React.createElement(
                'button',
                {
                  ...
                  title:
                    '到酒館的「📖 藏書」打開「' +
                    one.id +
                    '」——在那裡改標題、關鍵字、優先序與**內容**（房間這一頁的內容是唯讀的）',
                  'aria-label': '到藏書頁修改「' + one.id + '」',
                  onClick: function (event) {
                    // 這一顆是**動作**不是摺疊：不要讓它順手把這一列展開／收起。
             ...
             *   > 然後摺疊功能改為整條都可以摺疊
             *
             * 三個細節，缺一都會讓人覺得「按了沒反應」或「怎麼突然跳開」：
             *   1. `onClick` ＋ `stopRowToggle` 包住的**控制項**：列上的開關與位置
             *      選單要自己吃掉 click，不然按它們會順手把整列展開／收起。
             *   2. **鍵盤**：`role="button"` ＋ `tabIndex` ＋ Enter／Space——
             *      「整條可以按」的東西用滑鼠做得到、用鍵盤做不到是壞的。
             *   3. `aria-expanded`：讀屏器要知道這一列是展開的還是收起的。
             */
            return React.createElement(
              'div',
              {
                key: one.id,
                className: 'dsh-tv-bookRow dsh-tv-bookRowClick',
                role: 'button',
                tabIndex: 0,
                'aria-expanded': opened ? 'true' : 'false',
                'aria-label': one.id + '：' + (opened ? '收起條目' : '展開條目（看內容與調這一間房的優先序）'),
                title: opened ? '按一下收起「' + one.id + '」的條目' : '按一下展開「' + one.id + '」的條目',
                onClick: function () {
                  loadContent(one.id)
                },
                onKeyDown: function (event) {
    ...
    module.exports = {
      ...
      /**
       * **跳到某一本書**的那條路（2.6.72 加回來的）。
       *
       * ⚠️ 匯出給測試是必要的：它是**三件模組層級狀態**的組合（交接格、
       * 分區、主面板），而「少做一件」的症状是**什么都没有發生**
       * （2.6.66 實際回報過「去改它沒有跳過去」）——那在畫面上看不出來。
       */
