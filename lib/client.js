window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    var CLIENT_BUILD = 'tavern-client-2.6.9'
    ...
    function MapTavernActions(props) {
      ...
      return React.createElement(
        ...
        React.createElement(MapSelect, {
          ...
          options: [
            { value: 'none', label: '全關（預設）——不能讀檔、不能跑指令' },
            { value: 'read', label: '只讀——讀檔／找檔（read、glob、grep）' },
            { value: 'write', label: '讀＋寫——加上改檔（write、edit）' },
            { value: 'web', label: '只讀＋上網——加上網路（web_search、web_fetch）' },
            { value: 'all', label: '全部——跟一般 agent 一樣（含跑指令）' },
          ],
          ...
          currentChatTab === 'room'
            ? [
                ...
                React.createElement(MapSelect, {
                  ...
                  hint: '「聽酒館的」＝沿用 ⚙️ 設定 那一層（預設是「全關」）。選了就馬上生效。',
                  options: [
                    { value: 'inherit', label: '聽酒館的（預設）' },
                    { value: 'none', label: '全關——不能讀檔、不能跑指令' },
                    { value: 'read', label: '只讀——read、glob、grep' },
                    { value: 'write', label: '讀＋寫——加上 write、edit' },
                    { value: 'web', label: '只讀＋上網——加上 web_search、web_fetch' },
                    { value: 'all', label: '全部——跟一般 agent 一樣' },
                  ],
                  onChange: function (value) {
