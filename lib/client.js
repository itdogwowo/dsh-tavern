window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    var CLIENT_BUILD = 'tavern-client-2.6.4'
      ...
      function build() {
        ...
        return rpc('chat.create', { id: tavernId, character: chosen.id, name: state.draft.name })
          ...
          .then(function (created) {
            ...
            currentChat = { character: created.character, name: created.name, file: created.file }
            bumpActiveVersion()
      ...
      if (!state.loaded) {
        ...
        if (currentZone === 'rooms') {
          ...
          body = React.createElement(
            ...
            // 快速入口：進到一間酒館的人，十之八九是要聊天或改卡，
            // 不該逼他先發現上面那一列分區（`redesign.md` §3.2 的「跳去別區」）。
            React.createElement(
              'div',
              { className: 'dsh-tv-quick' },
              [
                ['rooms', '💬 包廂', 'chats', '份對話'],
                ['cast', '🎭 卡司', 'characters', '張卡'],
                ['books', '📖 藏書', 'worldbooks', '本'],
              ].map(function (one) {
                var n = counts[one[2]]
                return React.createElement(
                  'button',
                  {
                    key: one[0],
                    type: 'button',
                    className: 'dsh-tv-quickBtn',
                    onClick: function () {
                      currentZone = one[0]
                      renderPage()
                    },
                  },
                  React.createElement('span', { className: 'dsh-tv-quickLabel' }, one[1]),
                  React.createElement(
                    'span',
                    { className: 'dsh-tv-quickHint' },
                    String(typeof n === 'number' ? n : 0) + ' ' + one[3],
                  ),
                )
              }),
            ),
    ...
    function TavernChatPage() {
       ...
       */
      function messageRow(key, message, avatarUrl, thinking) {
            ...
            : currentChatTab === 'file'
              ? [
                  React.createElement(
                    'p',
                    { className: 'dsh-tv-note' },
                    '對話檔：chats/' + selected.character + '/' + selected.file,
                  ),
                  React.createElement(
                    'p',
                    { className: 'dsh-tv-note' },
                  ...
                  React.createElement(
                    'p',
                    { className: 'dsh-tv-note' },
                    '這間房的插圖：art/chats/' + selected.character + '/' + selected.name + '/',
                  ),
                  React.createElement(
                    'p',
                    { className: 'dsh-tv-note' },
              ...
              : [
                  ...
                  React.createElement(
                    ...
                    React.createElement(
                      ...
                      React.createElement(
                        MapBtn,
                        { primary: true, disabled: chat.busy, onClick: submit },
                        chat.busy ? '回覆中…' : '送出',
                      ),
                      chat.busy ? React.createElement(MapBtn, { onClick: stop }, '停下來') : null,
                      React.createElement(
                        'span',
